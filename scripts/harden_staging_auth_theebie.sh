#!/usr/bin/env bash
# Enable registration auth on theebie.de and verify from maintainer machine.
set -euo pipefail
...
scp scripts/remote/harden_platform_auth_theebie.sh "$HOST:$REMOTE_ROOT/scripts/remote/"
ssh "$HOST" "chmod +x '$REMOTE_ROOT/scripts/remote/harden_platform_auth_theebie.sh' && bash '$REMOTE_ROOT/scripts/remote/harden_platform_auth_theebie.sh'"
...
python scripts/verify_registration_auth.py "$API_URL"
AGENTSWARM_VERIFY_QUICK=1 python scripts/verify_production_staging.py "$API_URL"

echo "Staging auth hardening verified: $API_URL"
