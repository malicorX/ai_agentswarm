# Phase 16 close-out: dispatch migration phase 3 on staging (P16.11).
...
for key, expected in (
    ...
    if actual != expected:
        print(f'expected assignment.{key}={expected!r}, got {actual!r}', file=sys.stderr)
        raise SystemExit(1)
if assignment.get('pull_for_maintainer_scripts') is not True:
    print('expected assignment.pull_for_maintainer_scripts=True', file=sys.stderr)
    raise SystemExit(1)
...
Write-Host "Phase 16 close-out checks OK. Tag with:"
Write-Host "  git tag v0.17.0-phase16 && git push origin v0.17.0-phase16"
